...
static void mp_cache_hash_token(const char *token, uint8_t out_hash[MP_CACHE_TOKEN_HASH_SIZE]) {
    mp_cache_sha256((const uint8_t *)token, token == NULL ? 0u : strlen(token), out_hash);
}

static mp_cache_security_status_t mp_cache_security_issue_token(char *out_token, size_t out_capacity) {
    uint8_t random_bytes[32];

    if (out_token == NULL || out_capacity < MP_CACHE_TOKEN_TEXT_CAP) {
        return MP_CACHE_SECURITY_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (mp_cache_hex_encode(random_bytes, sizeof(random_bytes), out_token, out_capacity) != 0) {
        return MP_CACHE_SECURITY_STATUS_IO_ERROR;
    }
...
int mp_cache_secret_resolve(const char *secret_ref, char *out_secret, size_t out_capacity) {
    ...
    if (strncmp(secret_ref, "env:", 4u) == 0) {
        value = getenv(secret_ref + 4u);
        if (value == NULL || *value == '\0') {
            errno = ENOENT;
            return -1;
        }
        (void)snprintf(out_secret, out_capacity, "%s", value);
        return 0;
    }

    if (strncmp(secret_ref, "file:", 5u) == 0) {
        const char *path = secret_ref + 5u;
        if (*path != '/') {
            errno = EINVAL;
            return -1;
        }
...
mp_cache_security_status_t mp_cache_security_authenticate(
    ...
    if (security == NULL || token == NULL || *token == '\0' || out_principal == NULL) {
        return MP_CACHE_SECURITY_STATUS_INVALID_ARGUMENT;
    }

    mp_cache_hash_token(token, token_hash);
...
mp_cache_security_status_t mp_cache_security_register_client(
    ...
    if (security == NULL || out_token == NULL || role == MP_CACHE_ROLE_NONE || !mp_cache_client_id_is_valid(client_id)) {
        return MP_CACHE_SECURITY_STATUS_INVALID_ARGUMENT;
    }
    ...
    status = mp_cache_security_issue_token(out_token, out_token_capacity);
    if (status != MP_CACHE_SECURITY_STATUS_OK) {
        return status;
    }
    ...
    record->role = role;
    mp_cache_hash_token(out_token, record->token_hash);
    return MP_CACHE_SECURITY_STATUS_OK;
}
...
mp_cache_security_status_t mp_cache_security_rotate_client_token(
    ...
    if (security == NULL || out_token == NULL || !mp_cache_client_id_is_valid(client_id)) {
        return MP_CACHE_SECURITY_STATUS_INVALID_ARGUMENT;
    }
