...
# Runs unit test binaries under Valgrind and records host blockers clearly.
#
# Usage examples:
#   ./scripts/test-valgrind.sh
#   ./scripts/test-valgrind.sh .tmp/test-reports

if [ "${1:-}" = "--help" ]; then
  cat <<'EOF'
Usage: ./scripts/test-valgrind.sh [report-dir]
