...
static void configure_temp_paths(mp_cache_config_t *config, const char *suffix) {
    assert(config != NULL);
    (void)snprintf(config->data_directory, sizeof(config->data_directory), ".tmp/storage-test-%s-data", suffix);
    (void)snprintf(config->export_directory, sizeof(config->export_directory), ".tmp/storage-test-%s-exports", suffix);
    ...
    (void)snprintf(config->log_directory, sizeof(config->log_directory), ".tmp/storage-test-%s-logs", suffix);
}
