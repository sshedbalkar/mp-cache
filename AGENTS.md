...
- Any new or updated script under `scripts/` must keep top-level comments with a short purpose statement and a `Usage examples:` block; helper library scripts may show sourcing examples instead of direct execution. Every script must also print usage and exit successfully when invoked directly with `--help`.
